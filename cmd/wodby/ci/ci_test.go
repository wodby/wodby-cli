...
func TestPushAndReleaseAlias(t *testing.T) {
	...
	for _, name := range []string{"push", "release"} {
		t.Run(name, func(t *testing.T) {
			...
			if err := os.WriteFile(configPath, []byte(`{"BuiltServices":[{"Name":"php","Image":"registry.example.com/app:42","Released":false},{"Name":"nginx","Image":"registry.example.com/nginx:42","Released":false}],"AppBuild":{"gitRefType":"BRANCH","gitRef":"main"}}`), 0600); err != nil {
				t.Fatal(err)
			}
			...
			want := "push registry.example.com/app:42\ntag registry.example.com/app:42 registry.example.com/app:latest\npush registry.example.com/app:latest\ntag registry.example.com/app:42 registry.example.com/app:main\npush registry.example.com/app:main\n"
			if string(calls) != want {
				t.Fatalf("Docker calls:\n%s\nwant:\n%s", calls, want)
			}
			// The legacy Released key remains readable by deploy and older CLIs.
			state, err := os.ReadFile(configPath)
			if err != nil {
				t.Fatal(err)
			}
			var config types.Config
			if err := json.Unmarshal(state, &config); err != nil {
				t.Fatal(err)
			}
			if !config.BuiltServices[0].Released || config.BuiltServices[1].Released {
				t.Fatalf("unexpected push state: %+v", config.BuiltServices)
			}
			if !strings.Contains(string(state), `"Released": true`) {
				t.Fatalf("legacy state key is missing: %s", state)
			}
		})
	}
}
